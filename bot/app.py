...
def main():
    ...
    def start(message):
        ...
        welcome_text = (
            ...
            "• `/image` - generate image with prompt\n\n"
    ...
    def generate_image(message):
        prompt = (message.text or "").replace("/image", "").strip()
        if not prompt:
            bot.reply_to(message, "⚠️ Please provide a prompt.", parse_mode="Markdown")
            return

        bot.send_chat_action(message.chat.id, 'upload_photo')

        try:
            image_url = ai_client.get_image(prompt)
            ...
            bot.send_photo(message.chat.id, photo=file_obj)
            logger.info(f'AI Image generated | Chat ID: {message.chat.id} | Image URL: {image_url}')

        except Exception as e:
            logger.error(f"Image generation error | Chat ID: {message.chat.id} | Error: {e}")
            bot.reply_to(message, "😕 Could not generate or send image. Try again later.", parse_mode="Markdown")
